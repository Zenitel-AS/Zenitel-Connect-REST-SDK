...
namespace Wamp.Client
{
    public partial class WampClient
    {
        ...
        /// <summary>
        /// This method sends a call action request to the connected Zenitel Connect Platform. It will return the outcome of the request
        /// execution in the wamp_response class.
        /// </summary>
        /// <param name="callId">This parameter specifies the call id for which the action should be excuted.</param>
        /// <param name="action">This parameter specifies if the it is a csll setup or call answer request.</param>
        /// <returns name="wamp_response">This class contains the result of the request execution and a describing text in case the
        /// request fails.
        /// </returns>
        /***********************************************************************************************************************/
        public wamp_response PostCallsId(string callId, string action)
        /***********************************************************************************************************************/
        {
            wamp_response wampResp = new wamp_response();
            ...
            try
            {
                ...
                if (rpcCallback.RespRecv)
                {
                    ...
                    {
                        wampResp.WampResponse = ResponseType.WampRequestSucceeded;
                        wampResp.CompletionText = "PostCallsId sucessfully completed.";
                    }
                    else
                    {
                        wampResp.WampResponse = ResponseType.WampRequestFailed;
                        wampResp.CompletionText = "PostCallsId failed: " + rpcCallback.CompletionText;
                    }
                }
                else
                {
                    wampResp.WampResponse = ResponseType.WampNoResponce;
                    wampResp.CompletionText = "PostCallsId failed. No response from WAMP.";
                }
            }
            catch (Exception ex)
            {
                OnChildLogString?.Invoke(this, "Exception in PostCallsId: " + ex.ToString());
            }
